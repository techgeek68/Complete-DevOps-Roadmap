...
```
- Lets traffic in and allows Nginx to talk to upstreams when SELinux is enforced.
...
- 502 Bad Gateway: Upstream down, DNS wrong, firewall/SELinux blocking. Test with curl from the Nginx host.
- 504 Gateway Timeout: Upstream slow; raise proxy_read_timeout or fix backend performance.
- 413 Request Entity Too Large: Increase client_max_body_size.
- 499 Client Closed Request: User cancelled or closed connection; often harmless.
- TLS issues: Check full chain, SNI, and OCSP trusted chain; certbot renew logs help.
