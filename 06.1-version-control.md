...
```
git remote -v
git remote add origin git@github.com:techgeek68/DevTestJAVA.git
...
```
         ...
         Commit   | | Update
          ...
          Push  | | Pull                | | Push/Pull
                | v                     | v 
            +------------------+  +------------------+
(User 2)    | Local Repository |  | Local Repository |  (User 3)
            +------------------+  +------------------+
               ^ |                    ^ |
...
```
git pull origin main
# edits product-list.js
git add product-list.js
git commit -m "feat: add price range filter"
git push origin main
```

- Carol (pushes before Bob)
```
git fetch origin
# edits styles.css
git add styles.css
...
- Click “New”
- Name: devtestjava (and optionally DevTestPython, DevTestPHP)
...
## Install Git and Perform Core Operations
