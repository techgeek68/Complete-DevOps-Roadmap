...
**A. Remove/Stop Apache (if needed)**
 - If multiple services use the same port number on the same server, issues will occur. There are many ways to manage that, but for now, let’s just disable, stop, or uninstall Apache.
...
## 9. Nginx Web Server
 ...
 - NGINX provides a high-performance, event-driven architecture with the ability to efficiently handle extreme concurrent loads. Unlike traditional servers (like older Apache), which spawn a new process or thread for every connection, thereby consuming resources, NGINX uses a streamlined process model.
...
server {
    listen 80;
    server_name 192.168.64.4 www.devops.com;
